...
/** The types of charts that are supported */
export type ChartType = 'line' | 'bar' | 'horizontalBar' | 'radar' | 'pie' | 'doughnut' | 'polarArea' | 'bubble' | 'scatter';
...
export class ChartDirective implements OnDestroy, OnChanges, OnInit {
	...
	set data(value) { this._data = CoercionHelper.coerceArray(value); }
	private _data: Array<any>;

	@Input()
	get datasets(): Array<any> { return this._datasets; }
	set datasets(value) { this._datasets = CoercionHelper.coerceArray(value); }
	private _datasets: Array<any>;

	@Input()
	get labels(): Array<any> { return this._labels; }
	set labels(value) { this._labels = CoercionHelper.coerceArray(value); }
	...
	@Input()
	get colors(): Array<any> { return this._colors; }
	set colors(value) { this._colors = CoercionHelper.coerceArray(value); }
	private _colors: Array<any>;

	@Input()
	get legend(): boolean { return this._legend; }
	...
	private getDatasets(): any {
		...
		if (!datasets) {
			throw new Error(`Charts configuration error, data or datasets field are required to render char ${this.chartType}`);
		}
...
function generateColor(index: number): number[] {
	return ChartDirective.defaultColors[index] || getRandomColor();
}
...
function generateColors(count: number): Array<number[]> {
	...
	for (let i = 0; i < count; i++) {
		colorsArr[i] = ChartDirective.defaultColors[i] || getRandomColor();
	}
	return colorsArr;
}
