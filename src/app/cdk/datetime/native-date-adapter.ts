...
import { ObjectHelper } from '@app/cdk/helpers';
...
/** Adapts the native JS Date for use with components that work with dates. */
@Injectable()
export class NativeDateAdapter extends DateAdapter<Date> {
	...
	constructor ( @Optional() @Inject(DATE_LOCALE) matDateLocale: string) {
		super();
		super.setLocale(matDateLocale);

		// IE does its own time zone correction, so we disable this on IE.
		// TODO: replace with !platform.TRIDENT, logic currently duplicated to avoid breaking
	...
	format(date: Date, displayFormat: Object): string {
		...
		if (SUPPORTS_INTL_API) {
			if (this.useUtcForDisplay) {
				...
				displayFormat = ObjectHelper.extendObject({}, displayFormat, { timeZone: 'utc' });
			}
